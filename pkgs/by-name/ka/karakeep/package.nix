...
let
  # pnpm 10 hashes patch files with sha256, but karakeep's lockfile was
  # generated by pnpm 9 (26-char base32 hashes). Rewrite the recorded patch
  # hashes to their sha256 equivalents so the frozen install accepts them.
  # Values from `pnpm@10 install --lockfile-only` on the two patched packages.
  fixupPnpmPatchHashes = ''
